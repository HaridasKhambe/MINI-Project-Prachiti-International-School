...
const events = () => {
  useEffect(() => {
    document.title = "Events | JSPM College";
  }, []);
    ...
    <div>
      ...
      <div className='flex flex-col my-5 mx-4 md:mx-10 space-y-6'>
                  <div className=' justify-center text-center'>  {/* title heading */}
                        <h1 className='text-4xl text-[var(--primary-color)] font-medium  pb-2 border-b-3 border-[var(--golden-border-b)] '>Events</h1>
                  </div>
                ...
                className='w-full h-full p-8 space-y-8 md:px-20'>   {/*libry content - start */}
                    ...
                    >  {/*images */}
                        <div className=''>  {/* title heading */}
                              <h2 className='inline-flex text-2xl text-[var( --footer-bg)] font-medium  pb-2 border-b-2 border-[var(--golden-border-b)] '>Memorable College Event</h2>
                        </div>
                        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mt-6 lg:px-20'>
                        {galleryData.events.items.map((item, index) => (
